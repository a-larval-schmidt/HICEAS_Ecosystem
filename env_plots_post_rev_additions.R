...
#ik<-read.csv("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Data/malolo_HICEAS_metadata.xlsx - Sheet1.csv")
ik<-read.csv("A:/My Drive/crusies/HICEAS_23/SE2303 ID Log - ikmt_metadata.csv")
...
#justin &kym's points
#jk<-read.csv("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Data/HICEAS_IKMT_Locations_Times_KY_JS.xlsx - Sheet1.csv")
#jk<-jk%>%dplyr::select(c(Station, Lat_DD, Lon_DD))
#ijk<-left_join(ik,jk, by=join_by(Station))
#ik<-ijk %>% mutate(lat_dd= Lat_DD)
#ik<-ik %>% mutate(lon_dd= Lon_DD)
ik<-ik %>% mutate(date= ymd(Date,tz="HST"))
ik<-ik %>% mutate(tow_duration= Time.Out.of.Water-Time.In..Local.)
#sst data extraction####
#DONT USE THIS ONE, IT DROPPED 6 STATIONS: ikd<-read.csv("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Data/env_pplots_data_w_sse.csv")#ik[,1]<-NULL
#legend_title<-"SST (°C)"
...
chla_df<-read.csv("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Data/chla_extraction_glorys_4km.csv")
...
final_ik3_dt <- spatial_matches_dt %>%as_tibble() %>% 
  ...
  slice(1) %>% 
  ungroup() %>%
  dplyr::select(Station,chla_value, date_diff)
...
#add in fish info#####
larv<-read.csv("A:/My Drive/crusies/HICEAS_23/SE2303 ID Log - ID&Length.csv")
se23<-left_join(ik3, larv,by=join_by("Station"=="station"))
#if running w/o sst, chlase23<-left_join(ik, larv,by=join_by("Station"=="station"))
se23<-se23%>%
...
fish_sil_paths<-c(
  "Aprion"="C:/Users/andrea.schmidt/Pictures/Aprion-virescens_6mm_HLIP-555_TC8604_phylopic.png",
  "Etelinae"="C:/Users/andrea.schmidt/Pictures/tow2eteline_phylopic.png",
  "Acanthocybium"="C:/Users/andrea.schmidt/Pictures/ono_phylopic.png",
  "Coryphaena"="C:/Users/andrea.schmidt/Pictures/IMG_7366_mahi_phylopic_se2503_st9_sil.png",
  "Katsuwonus"="C:/Users/andrea.schmidt/Pictures/IMG_7679_k_pelamis_se2503_st23_phylopic_sil.png",
  "Thunnus"="C:/Users/andrea.schmidt/Pictures/Thunnus_Larva_HICEAS_St3_phylopic.png",
  "Istiophoridae" ="C:/Users/andrea.schmidt/Pictures/Billfish_PC_JPerelman_phylopic_asis.png"
  )
adult_etelines<-c(
  "Etelinae"="C:/Users/andrea.schmidt/Pictures/paka_phylopic.png",
  "Aprion"="C:/Users/andrea.schmidt/Pictures/uku_phylopic.png")
taxa_to_plot <- list(
  family = c("Istiophoridae"),
...
df<- se23 %>% filter(subfamily == "Etelinae")
...
all_density_data <- list(tuna, marlin,df,mahi,uku, aku,ahi, ono, sword)
...
larv2 <-rbind(tuna, marlin,df,mahi,uku, aku,ahi, ono)
...
for (level in names(taxa_to_plot)) {
  for (fish_taxa in taxa_to_plot[[level]]) {
    ...
    #add fish logo
    current_logo_path <- fish_sil_paths[fish_taxa]
    if (!is.na(current_logo_path)) {
      logo_df <- data.frame(x = 204,y = 29.8,image = current_logo_path)
    }
    ...
    full_path <- file.path("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/", summary_filename)
    capture.output(
    ...
    ggsave(paste0(fish_taxa, "_length_histogram.png"), plot = hist, width = 4, height = 4,
           path="A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Plot Figures/")
    ...
    p16 <- ggplot(data = df, aes(x = day_of_year, y =density)) +
      ...
      labs(x = "Day of Year",
           y = bquote(paste(.(fish_taxa)," Density (fish 1000" ~ m^{-3} ~ ")")))+
    ...
    p17 <- ggplot(data = df, aes(x = day_of_year, y =density)) +
      geom_point(size = 3)+
      labs(x = "Day of Year",y = bquote(paste(.(fish_taxa)," Density (fish 1000" ~ m^{-3} ~ ")")))+
    ...
    p2 <- ggplot(data = df, aes(x = sst, y = density)) +geom_point(size=3, alpha=0.5)+
      ...
      labs(x =expression("SST (" * degree * "C)"),
           y = bquote(paste(.(fish_taxa)," Density (fish 1000" ~ m^{-3} ~ ")")))+theme_bw()+
      theme(plot_theme)
    p2<-p2+geom_point(data=anti_df, aes(x = sst, y = (density*0)), shape=0, size=2.5, alpha=0.5)
    ggsave(paste0(fish_taxa, "_density_vs_sst.png"), plot = p2, width = 4, height = 4,path="A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Plot Figures/")
    ...
    p3 <- ggplot(data = df, aes(x =chla_value, y = density)) +
      ...
      labs(x = expression("Chlorophyll Concentration" * "\n" * "(mg" ~m^{-3}~")"),
           y = bquote(paste(.(fish_taxa),"  Density (fish 1000" ~ m^{-3} ~")")))+
      theme_bw()+theme(plot_theme)
    p3<-p3+geom_point(data=anti_df, aes(x = chla_value, y = (density*0)), shape=0, size=2.5, alpha=0.5)
    ggsave(paste0(fish_taxa, "_density_vs_chla.png"), plot = p3, width = 4, height = 4,path="A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Plot Figures/")
    ...
    map_plot<-ggplot() +
      ...
      geom_point(data=anti_df,mapping=aes(y=lat_dd, x=lon_dd, color=day_of_year), shape=1, size=2)+
      coord_sf(xlim=c(178.4,204.5), ylim=c(18.6, 31.4)) +
      scale_x_continuous(breaks = round(seq(min(Hawaii_df$x, na.rm=T), max(Hawaii_df$x,na.rm=T), by = 5),1),
                        labels = function(x) { x - 360 })+
      scale_color_gradientn(colors=c("yellow","orange","orangered","red","red2","red3","red4"),na.value="gray 90",name="Day of Year",limits=c(min(se23$day_of_year, na.rm=T),max(se23$day_of_year, na.rm=T)))+
      ...
      #ggimage::geom_image(data = logo_df,aes(x = x, y = y, image = image),size = 0.15,by = "width",asp = 1.5)+
      annotate("point", x=177.8, y=18.35, color="black",shape=1, size=2)+
      rphylopic::add_phylopic(x = 203, y = 31.3,height = 1.5,name=fish_taxa, alpha = 1)+
      ylab(expression("Latitude (" * degree *"N)"))+xlab(expression("Longitude (" * degree * "W)"))+
      guides(fill="none",color = guide_colorbar(order = 1),size = guide_legend(order = 2))
    if (fish_taxa %in% c("Etelinae", "Aprion")) {
      current_alt_path <- adult_etelines[fish_taxa]
      alt_logo_df <- data.frame(
        x = 203,y = 31.3,image = current_alt_path)
      map_plot <- map_plot +
        ggimage::geom_image(data = alt_logo_df,aes(x = x, y = y, image = image),
                            size = 0.15,by = "width",asp = 1.5)
    }
    ggsave(paste0(fish_taxa, "_map_newcolors.png"),
           plot = map_plot,
           path = "A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Plot Figures/",
           width = 8, height = 6)
    ...
    ggsave(
      ...
      path = "A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/combined figures/")
  }
}
...
for (level in names(taxa_to_plot)) {
  for (fish_taxa in taxa_to_plot[[level]]) {
    ...
    # Helper function to easily format the min and max into a "min - max" string
    get_range <- function(vec, digits = 1) {
      if(all(is.na(vec))) return("NA") # Failsafe if a column is totally empty
      paste0(round(min(vec, na.rm = TRUE), digits), " - ", round(max(vec, na.rm = TRUE), digits))
    }
    
    # Combine Hawaiian name with Scientific name for the table (optional, adjust as needed)
    hawaii_name <- hawaiian_names[fish_taxa]
    
      name_col_1 <- ifelse(is.na(hawaii_name), "-", hawaii_name)
      name_col_2 <- full_scientific_name
    # Create a 1-row dataframe for this specific taxa
    temp_df <- data.frame(
      Hawaiian_Name_Col = name_col_1,
      Taxa_Col          = name_col_2,
      Taxa_Level        = level,        # <--- Crucial: Saves the level for the italics rule!
      Total_larv        = sum(distinct(df, station, total)$total, na.rm = TRUE),
      Occurrences_Col   = length(unique(df$station)),
      Size_Col          = get_range(df$length, 1),       
      SST_Col           = get_range(df$sst, 1),
      Chla_Col          = get_range(df$chla_value, 3),
      stringsAsFactors  = FALSE
    )
        summary_list[[fish_taxa]] <- temp_df
    mean_df<-data.frame(
      Taxa_Col          = name_col_2,
      mean_sst          = round(mean(df$sst, na.rm = TRUE),1),
      mean_chla         = round(mean(df$chla_value, na.rm = TRUE),3),   
      mean_density      = round(mean(df$density, na.rm=T),3),
      density_range     = get_range(df$density,3),
      mean_doy          =round(mean(df$day_of_year, na.rm=T),0),
      mean_length       =round(mean(distinct(df, station, length)$length, na.rm = TRUE),1))
    mean_list[[fish_taxa]]<-mean_df
  }
}

intext_df <- dplyr::bind_rows(mean_list)
table2_df <- dplyr::bind_rows(summary_list)

# #hiceas sampling map#####
# ijk <- ijk %>%mutate(month = lubridate::month(ymd(Date),label=T))
# ijk<-ijk %>% mutate(lat_dd= Lat_DD)
# ijk<-ijk %>% mutate(lon_dd= ifelse(Lon_DD<0, Lon_DD+360, Lon_DD))
# fig1<-ggplot() +
#   geom_raster(data = Hawaii_df, aes(x = x, y = y, fill = z)) +labs(fill = "Depth (m)")+
#   scale_fill_gradient(high = "lightskyblue1", low = "lightsteelblue4",limits=c(-10000,0),
#                       na.value=(color="#003300"),guide="none")+
#   geom_point(data=ijk,mapping=aes(y=lat_dd, x=lon_dd,color=month),shape=19,size=3) +
...
# ggsave("fig1.png",
#        plot = fig1,
#        path = "A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Map Figures 2/",
#        width = 8, height = 6)
# 
#   
# #doy/ density##########
...
# summary_file <- file.path("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/cor_tests.txt")
...
# library(visreg)
# library(coefplot)
# output_dir <- "A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Stat Figures"
...
# 
# 
# #in/out of monument determination#####
# library(sf)
# PMNM<-read_sf("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/Data/PMNM_Shp_File/hi_noaa_nwhi_papahanaumokuakea.shp")
...
# PMNM_360 <- st_make_valid(PMNM_360)
# intersection_result <- st_within(aku_sf, PMNM_360, sparse = FALSE)
# se23<-aku%>%mutate(Location_Assessment=ifelse(rowSums(intersection_result) > 0, "in", "out"))
# 
# #output_dir<-"A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/in_out plots"
# 
# #in/out stats#########################
# df <- se23 %>% filter(family == "Scombridae")
# anti_df <- se23 %>% filter(family != "Scombridae")
# df$pa<-1#present
# anti_df$pa<-0#absent
# anti_df$density<-0#absent
# df<-rbind(df,anti_df)
# ggplot() + geom_density(data=df, aes(x = density))
# ggplot() + geom_density(data=df, aes(x = pa))
# my_comparisons <-c("in", "out")
# ggplot()+geom_boxplot(data=df, aes(x=Location_Assessment, y=density))+labs(title="Scombridae")+
#   stat_compare_means(
#     method = "t.test",
#     ref.group = my_comparisons, # Compare each group to the overall mean of 'density'
#     label = "p.signif",  # Use significance symbols (e.g., *, **, ***)
#     label.y.npc = "top", # Position the labels at the top of the plot area
#     vjust = 0.5)
# t.test(pa ~ Location_Assessment, data=df)
# #welchʻs t test (PA by Location
# t.test(pa ~ Location_Assessment, data=df, var.equal=FALSE)
# wilcox.test(pa ~ Location_Assessment, data=df) 
# model<-mgcv::gam(data=df, pa~Location_Assessment+s(sst, k=4)+ s(log(chla_value), k=4) +
#                    factor(month) + te(lon_dd, lat_dd, k=4),family = binomial(link="logit"))
# 
# model<-mgcv::gam(data=df, density~Location_Assessment+s(sst, k=4)+ s(log(chla_value), k=4) +
#                    factor(month) + te(lon_dd, lat_dd, k=4),family = tw(link="log"))
# 
# summary(model) 
...
# #in_out loop binomial#######
# output_dir<-("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/in_out_bino")
# if (!dir.exists(output_dir)) {
#   dir.create(output_dir)}
# 
# for (level in names(taxa_to_plot)) {
...
#     tryCatch({
#       #4.5 Fit binomial model
#       model <- gam(pa ~ Location_Assessment+s(sst, k=4) + s(log(chla_value), k=4) +
#                      factor(month) + te(lon_dd, lat_dd, k=4), 
#                    family = binomial(link="logit"), data = df)
#       
#       # 5. Save Model Summary and ANOVA to a Text File using sink()
...
#       plot(cooks.distance(model), type="h", main = paste("Cook's Distance -", full_scientific_name)) 
#       
#       # Visualization of monument effect
#       visreg(model, "Location_Assessment", scale = "response", ylab = "presence/absence (Response Scale)", 
#              xlab = "Inside or ", alpha=0.01, rug=1, 
#              main = paste("Partial Effect of SST -", full_scientific_name))
#       
#       # Visualization of chla effect
#       visreg(model, "daily chla", scale = "response", ylab = "presence/absence (Response Scale)", 
...
#     })}}
# ##in out loop tweedie###############
# output_dir<-("A:/My Drive/crusies/HICEAS_23/Ichthyoplankton Projects/Ichthyoplankton Projects/in_out_tweed")
# if (!dir.exists(output_dir)) {
#   dir.create(output_dir)}
# 
# for (level in names(taxa_to_plot)) {
...
# ######by hand##############
# 
# df<- se23 %>% filter(family == "Istiophoridae")
# anti_df <- se23 %>% filter(family != "Istiophoridae")
# df$pa<-1#present
# anti_df$pa<-0#absent
# anti_df$density<-0#absent
# df<-rbind(df,anti_df)
# ggplot() + geom_density(data=df, aes(x = density))
# ggplot() + geom_density(data=df, aes(x = pa))
# ggplot()+geom_boxplot(data=df, aes(x=Location_Assessment, y=density))+labs(title="Istiophoridae")
#                       
# df<- se23 %>% filter(subfamily == "Etelinae")
# ggplot() + geom_density(data=df, aes(x = density))
# ggplot()+geom_boxplot(data=df, aes(x=Location_Assessment, y=density))+labs(title="Etelinae")
# 
# df<- se23 %>% filter(genus == "Coryphaena")
# ggplot() + geom_density(data=df, aes(x = density))
# ggplot()+geom_boxplot(data=df, aes(x=Location_Assessment, y=density))+labs(title="mahi")
# 
# uku<- se23 %>% filter(genus == "Aprion")
# ggplot() + geom_density(data=uku, aes(x = density))
# ggplot()+geom_boxplot(data=uku, aes(x=Location_Assessment, y=density))+labs(title="uku")
# 
# df<- se23 %>% filter(genus == "Katsuwonus")
# ggplot() + geom_density(data=df, aes(x = density))
# ggplot()+geom_boxplot(data=df, aes(x=Location_Assessment, y=density))+labs(title="aku")
# 
# ahi<- se23 %>% filter(genus == "Thunnus")
# ggplot() + geom_density(data=ahi, aes(x = density))
# ggplot()+geom_boxplot(data=ahi, aes(x=Location_Assessment, y=density))+labs(title="'ahi")
# 
# df<- se23 %>% filter(genus == "Acanthocybium")
# ggplot() + geom_density(data=df, aes(x = density))
# ggplot()+geom_boxplot(data=df, aes(x=Location_Assessment, y=density))+labs(title="ono")
# 
# 
