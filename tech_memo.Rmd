...
This section presents the results of our analysis, detailing the distribution and density of various fish taxa in relation to key environmental metrics.

### Plotting Taxa Results

The following plots show the density of each taxon in relation to Sea Surface Temperature (SST), Chlorophyll-a (Chla), and Sea Level Anomaly (SLA). All plots for a given metric share the same axis scales for ease of comparison.

```{r, results-code, warning=FALSE, message=FALSE, echo=FALSE}
# Load libraries
...
se23<-ik2%>%
  ...
  dplyr::select(Station,lat_dd,lon_dd,mean_flow, month, depth_m,total, toyos,tow_duration,
                sst,chla_daily,sla,FLAG, family, subfamily,genus, species,density)

# Global variables and data prep
# Recalculate global min/max for consistent scales
min_sst <- min(se23$sst, na.rm = TRUE)
max_sst <- max(se23$sst, na.rm = TRUE)
min_chla <- min(se23$chla_daily, na.rm = TRUE)
max_chla <- max(se23$chla_daily, na.rm = TRUE)
min_sla <- min(se23$sla, na.rm = TRUE)
max_sla <- max(se23$sla, na.rm = TRUE)
min_density <- min(se23$density, na.rm = TRUE)
max_density <- max(se23$density, na.rm = TRUE)

# Convert month to a factor with specific names and levels
se23$month <- factor(se23$month,
                     levels = c(7, 8, 10),
                     labels = c("July", "August", "October"))

# Named vector for Hawaiian names
hawaiian_names <- c(
  "Thunnus" = "ahi",
  "Aprion" = "uku",
  "Acanthocybium" = "ono",
  ...
  "Istiophoridae" = "a'u",
  "Xiphias" = "a'u kū",
  "Lutjanidae" = "snappers" # Added for completeness
)

# List of taxa to plot
taxa_to_plot <- list(
  family = c("Scombridae", "Istiophoridae", "Lutjanidae"),
...
for (level in names(taxa_to_plot)) {
  all_plots[[level]] <- list() # Create a sub-list for each level
  for (fish_taxa in taxa_to_plot[[level]]) {
    ...
    plot_list$boxplot <- ggplot(data = df, aes(x = month, y = density)) +
      geom_boxplot() +
      labs(title = paste(fish_taxa, "Density by Month"), y = "Density", x = "Month") +
      ylim(min_density, max_density) +
      theme_bw()
    
    # Plot 2: Density vs SST
    plot_list$sst <- ggplot(data = df, aes(x = sst, y = density)) +
      geom_point(size = 3) +
      labs(title = paste(fish_taxa, "Density vs SST"), y = "Density", x = "SST (°C)") +
      xlim(min_sst, max_sst) +
      ylim(min_density, max_density) +
      theme_bw()
    
    # Plot 3: Density vs Chla
    plot_list$chla <- ggplot(data = df, aes(x = chla_daily, y = density)) +
      geom_point(size = 3) +
      labs(title = paste(fish_taxa, "Density vs Chla"), y = "Density", x = "Chla (mg/m^3)") +
      xlim(min_chla, max_chla) +
      ylim(min_density, max_density) +
      theme_bw()
    
    # Plot 4: Density vs SLA
    plot_list$sla <- ggplot(data = df, aes(x = sla, y = density)) +
      geom_point(size = 3) +
      labs(title = paste(fish_taxa, "Density vs SLA"), y = "Density", x = "SLA Anomaly (m)") +
      xlim(min_sla, max_sla) +
      ylim(min_density, max_density) +
      theme_bw()
    
    # Plot 5: Map of density locations
    plot_list$map <- ggplot() +
      ...
      geom_point(data = df, aes(y = lat_dd, x = lon_dd, size = density, color = month), shape = 19, alpha = 0.5) +
      ...
      scale_color_manual(
        values = c("chartreuse4", "#800074", "#c99b38"),
        name = "Month",
        labels = c("July", "August", "October")
      ) +
      theme_bw() +
    ...
    all_plots[[level]][[fish_taxa]] <- plot_list
  }
}
```
### Taxa v.s. environment
####Scombridae
all_plots$family$Scombridae$boxplot
all_plots$family$Scombridae$sst
all_plots$family$Scombridae$chla
all_plots$family$Scombridae$sla
#####Aku
all_plots$genus$Katsuwonus$boxplot
all_plots$genus$Katsuwonus$sst
all_plots$genus$Katsuwonus$chla
all_plots$genus$Katsuwonus$sla

#####'Ahi
all_plots$genus$Thunnus$boxplot
all_plots$genus$Thunnus$sst
all_plots$genus$Thunnus$chla
all_plots$genus$Thunnus$sla

#####Ono
all_plots$genus$Acanthocybium$boxplot
all_plots$genus$Acanthocybium$sst
all_plots$genus$Acanthocybium$chla
all_plots$genus$Acanthocybium$sla

####Billfish
#####Istiophoridae
all_plots$family$Istiophoridae$boxplot
all_plots$family$Istiophoridae$sst
all_plots$family$Istiophoridae$chla
all_plots$family$Istiophoridae$sla

#####Xiphiidae
all_plots$genus$Xiphias$boxplot
all_plots$genus$Xiphias$sst
all_plots$genus$Xiphias$chla
all_plots$genus$Xiphias$sla

####Mahi
all_plots$genus$Coryphaena$boxplot
all_plots$genus$Coryphaena$sst
all_plots$genus$Coryphaena$chla
all_plots$genus$Coryphaena$sla


####Lutjanidae: Etelinae
all_plots$subfamily$Etelinae$boxplot
all_plots$subfamily$Etelinae$sst
all_plots$subfamily$Etelinae$chla
all_plots$subfamily$Etelinae$sla
#####uku
all_plots$genus$Aprion$boxplot
all_plots$genus$Aprion$sst
all_plots$genus$Aprion$chla
all_plots$genus$Aprion$sla


###Spatial Distributions
####Scombridae
all_plots$family$Scombridae$map
all_plots$genus$Katsuwonus$map
all_plots$genus$Thunnus$map
all_plots$genus$Acanthocybium$map
####Billfish
all_plots$family$Istiophoridae$map
all_plots$genus$Xiphias$map
####Mahi
all_plots$genus$Coryphaena$map
####Snappers
all_plots$subfamily$Etelinae$map
all_plots$genus$Aprion$map
#####Aprion v.s. all other Snappers (Lutjanus sp.)
```{r, results-uku vs to'ao, warning=FALSE, message=FALSE, echo=FALSE}
# --- Custom comparison plots for Aprion vs Lutjanidae ---
# Filter data for Aprion and the rest of the Lutjanidae family
df_aprion <- se23 %>% filter(genus == "Aprion")
df_lutjanidae_rest <- se23 %>% filter(family == "Lutjanidae" & genus != "Aprion")
...
comparison_map <- ggplot() +
  ...
  labs(fill = "Depth (m)") +
  scale_fill_gradient(high = "lightskyblue1", low = "lightsteelblue4", limits = c(-10000, 0), na.value = "black", guide = "none") +
  geom_point(data = df_aprion, aes(y = lat_dd, x = lon_dd, size = density), color = "red4", shape = 19, alpha = 0.7) +
  geom_point(data = df_lutjanidae_rest, aes(y = lat_dd, x = lon_dd, size = density), color = "blue", shape = 19, alpha = 0.7) +
  scale_size_continuous(range = c(1.5, 5)) +
  coord_sf(xlim = c(178.3, 204.6), ylim = c(18.6, 31.4)) +
  scale_x_continuous(breaks = round(seq(min(Hawaii_df$x), max(Hawaii_df$x), by = 5), 1)) +
  theme_bw() +
  ...
  ylab("Latitude") +
  xlab("Longitude") +
  labs(title = "Aprion vs. Lutjanidae Density Locations",
       color = "Taxa") +
  geom_path(data = re_ordered_EEZ, aes(x = Lon, y = X2), color = "white", linewidth = 1)
```